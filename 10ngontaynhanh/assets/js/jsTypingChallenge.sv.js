	...
	*/
	function calculateScores() {
		...
		var accuracy = Math.round(rightWords/(rightWords+wrongWords)*100,0);
		var officialWPM = Math.round( (rightKeystrokes/((rightKeystrokes+wrongKeystrokes)/rightWords)/5) );
		document.getElementById('test').hidden = true;
		getResultfield().innerHTML = "<br>" + 
			"<b>" + "*** K Ế T  Q U Ả ***" + "</b>" + "<br>"
			+ "Độ chính xác: <b>"+ accuracy + "%</b><br>"
			+ "Tốc độ: <b>" + officialWPM + " từ/phút</b><br>"
			+ "Số lỗi: <b>" + wrongWords + "</b><br><br>"
			+ '<a class="btn btn-success mb-2 mr-2 w-25" href="chamdiem.html?accuracy='+accuracy+'&speed='+officialWPM+'&wrong='+wrongWords+'">✔ Chấm điểm</a>'
			+ '<button type="submit" class="btn btn-primary mb-2 w-25" onclick="document.location.reload()">↺ Làm lại</button>';
			if(window.localStorage.score_sv==undefined) window.localStorage.score_sv = "[]";
			score = JSON.parse(window.localStorage.score_sv);
			score.push({
			...
			window.localStorage.score_sv = JSON.stringify(score);
	}
	...
	/*
  	...
  	*/
  	function gettingReadyToPlay() {
		...
		_isTimerOn = false;
  		clearTimeout(_t);
  		getTimerTextfield().innerHTML = _counter-1;
